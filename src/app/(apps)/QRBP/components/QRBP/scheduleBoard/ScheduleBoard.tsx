...
import useScheduleBoardProps from '@app/(apps)/QRBP/components/QRBP/scheduleBoard/useScheduleBoardProps'
...
type ScheduleBoardProps = {
  targetCars: any[]
  damageNameMaster: any[]
  query: any
  mode?: ScheduleBoardMode
}

const ScheduleBoard = (props: ScheduleBoardProps) => {
  const {mode = 'cr'} = props
  const {
    activeCars,
    targetDays,
    ...
    lastTouchedCarId,
    setlastTouchedCarId,
  } = useScheduleBoardProps(props, mode)
    ...
    <div className={`relative`}>
      ...
      <div className={`fixed right-5 bottom-5 z-50`}>
        <button
          ...
          disabled={movedCount === 0 || isPending}
          className={`
            flex items-center gap-2 rounded-full px-5 py-2.5
            text-sm font-medium text-white shadow-lg
            transition-all hover:shadow-xl
            ${movedCount > 0 && !isPending ? 'bg-blue-600 hover:bg-blue-700 cursor-pointer' : 'bg-gray-300 cursor-not-allowed'}
          `}
        >
          {isPending ? '保存中...' : '変更を保存'}
          {movedCount > 0 && !isPending && (
