...
const useNewMainBoardProps = props => {
  ...
  const activeCars = carsOnBoard.filter(car => car.crScheduledAt)
  const floatingCars = carsOnBoard.filter(car => !car.crScheduledAt)
  ...
  const handleDragEnd: (event: DragEndEvent) => void = (event: anyObject) => {
    ...
    if (activeId && overId) {
      const picked = {
        ...
        crScheduledAt: activeId.split('_')[2],
      }
      ...
      const current = {
        ...
        crScheduledAt: over?.id?.split('_')[1],
      }
...
export const create_cars_groupedBy_Damage_Date = (props: anyObject) => {
  ...
  carsOnBoard?.forEach(car => {
    ...
    const crScheduledAt = car.crScheduledAt ? formatDate(car.crScheduledAt) : 'unscheduled'
    obj__initializeProperty(cars_groupedBy_Damage_Date[damageNameMasterId], crScheduledAt, [])
    cars_groupedBy_Damage_Date[damageNameMasterId][crScheduledAt].push(car)
  })
