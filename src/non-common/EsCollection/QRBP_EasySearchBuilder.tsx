...
export const QrbpEasySearchBuilder = async () => {
  const car = async (props: easySearchType) => {
    ...
    const FINAL_CHECK_CUTOFF_DATE = new Date('2026-03-09')
    const alertThreshold = Days.day.add(today, 5)

    const Ex_exclusive_alert: exclusiveGroups = {
      needsFinalCheck: {
        label: '最終チェック未完了',
        notify: true,
        description: 'CR予定日が5日以内に迫っており、最終チェックが未完了の車両です',
        keyValueList: [{ key: 'needsFinalCheck', value: true }],
        exclusiveTo: QRBP?.cr,
        CONDITION: {
          ...commonNotCondition,
          orderedAt: { gte: FINAL_CHECK_CUTOFF_DATE },
          crScheduledAt: { not: null, lte: alertThreshold },
          Process: {
            none: { ProcessNameMaster: { name: '最終チェック' } },
          },
        },
      },
    }
    ...
    toRowGroup(1, dataArr, [
      ...
      { exclusiveGroup: Ex_exclusive_alert, name: `アラート` },
